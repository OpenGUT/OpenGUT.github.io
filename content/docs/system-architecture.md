...
A body-facing microphone captures abdominal acoustic signals, while an ambient-facing microphone records environmental noise. The two channels are sampled synchronously to support noise-aware processing. We use CMM-4030DB-26354-TR PDM MEMs microphone from Same Sky. Different versions of the OpenGut hardware may have different PDM microphones with similar frequency response, due to availability. 
...
8. While a mode is operating that led blinks. Each mode has a designated led that blinks. 
...
10. In mode 3 and 4, the respective audio file is played via the output.
